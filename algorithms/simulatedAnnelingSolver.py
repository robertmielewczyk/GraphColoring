...
class SimulatedAnnelingSolver(Algorithm):
    def __init__(self, graph, temperature=600, coolingRate=0.999, minTemperature=5):
        super().__init__()
        self.type = 'SimulatedAnneling'
        self.mask = self.generateRandomMask(graph)
        self.run(graph, temperature, coolingRate, minTemperature)

    def run(self, graph, temperature, coolingRate, minTemperature):
        start = time.time()
        iteracion = 0
    ...
    def neighbour(self, graph, mask):
        ...
        * Partition input sequence into a random number 
        ...
        '''
        neighbour_set = generateNeighboursSetSA(graph, mask)
